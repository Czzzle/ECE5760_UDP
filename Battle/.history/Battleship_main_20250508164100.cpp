...
//=========================== IRQ - BUtton ===================
bool left_button_state = true; // new variable with similar name!!!!

static void yellowButton_irq(void){
  ...
  // ------ MAIN FUNCTION HERE ----------
  bool state = gpio_get(BUT_PIN_Y);
  if (state!= left_button_state){
   
    printf("button state change from %b to %b", left_button_state, state);
...
int main()
{
  ...
  irq_set_exclusive_handler(ALARM_IRQ, yellowButton_irq) ;
