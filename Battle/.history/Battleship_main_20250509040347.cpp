...
enum PlayerState { YT, RR, RA }; // Your Turn, Ready Response, Ready Attack
  ...
  while (1)
  {
    ...
    if (playerBoard.game_status == GAME_STATUS::INITIAL)
    {
      if (yellow_pressed)
      {
        ...
        raw_sendd(GAME_STATUS::ONGOING, GRID_STATE::WATER, {(uint8_t)intcursor_x,(uint8_t)intcursor_y}, 1);
    ...
    else if (playerBoard.game_status == GAME_STATUS::ONGOING)
    {
      ...
      if (opponent_player == GAME_STATUS::ONGOING)
      {
        ...
        else if (my_state == RR && strstr(received_data, "GAME") != NULL)
        {
    ...
    else if(playerBoard.game_status == GAME_STATUS::WIN)
    {
