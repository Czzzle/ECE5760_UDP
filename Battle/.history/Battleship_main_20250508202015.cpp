...
//=========================== IRQ - Button ===================
bool yellow_button_state = false; // new variable with similar name!!!!
bool red_button_state = false;
bool yellow_pressed = false; // button change from 0 to 1 --> pressed = 1
...
static void button_irq(void)
{
  ...
  // ------ MAIN FUNCTION HERE ----------
  bool y_state = gpio_get(BUT_PIN_Y);
  bool r_state = gpio_get(BUT_PIN_R);

  // by default, we assume no button change from 0 to 1
  yellow_pressed = false;
  red_pressed = false;

  if (y_state != yellow_button_state)
  {
    ...
    yellow_button_state = y_state;
  }
  if (r_state != red_button_state)
  {
  ...
  while (1)
  {
    ...
    else if (playerBoard.game_status == GAME_STATUS::PLACE)
    {
      ...
      if (ctr_ship != 5)
      {
        ...
        if (select_flag == false)
        {
          printf("select_flag == false");
          val_ship = checkCursorOverShip(intcursor_x, intcursor_y);
          printf("val_ship = %d", val_ship);
          if (val_ship != 0 && prev_val != val_ship && yellow_pressed)
          {
            if (val_ship == 1)
            {
              shipname = "Carrier";
        ...
        else if (select_flag)
        {
          if (val_ship != 0)
          {
            ...
            // Check if we're within the grid (i.e., not the sentinel value 30,30)
            if (!yellow_pressed && !(grid_pos.x == 30 && grid_pos.y == 30))
            {
              char encoded[4]; // Enough space for something like "A10" + null terminator
              encodeCoord(grid_pos, encoded);
              printf("\nEncoded:%c %c, VAL_SHIP:%d", encoded[0], encoded[1], val_ship - 1);
            ...
            // Check if we're within the grid (i.e., not the sentinel value 30,30)
            else if (!red_pressed && !(grid_pos.x == 30 && grid_pos.y == 30))
            {
              char encoded[4]; // Enough space for something like "A10" + null terminator
              encodeCoord(grid_pos, encoded);
              printf("\nEncoded:%c %c, VAL_SHIP:%d", encoded[0], encoded[1], val_ship - 1);
      ...
      {
        printf("\nReached here finish placement");
        if (checkCursorOverStartGame(intcursor_x, intcursor_y) && yellow_pressed)
        {
          playerBoard.game_status = GAME_STATUS::ONGOING;
          printf("\nGAME_STATUS: %d", playerBoard.game_status_check());
          drawTextforShip(BLACK, BLACK, 1);
