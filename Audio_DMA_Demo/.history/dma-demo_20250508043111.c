...
unsigned short *address_pointer = &splash_data[0];
unsigned short *address_pointer2 = &boom_data[0];
...
int main()
{
    ...
    for (j = 0; j< boom_audio_len; j++){
        boom_data[k] = DAC_config_chan_A | (boom_audio[k] & 0x0FFF); //first convert to 12 bit then add config
    }

    // Select DMA channels, one pair for splash, one pair for boom
    int data_chan = dma_claim_unused_channel(true);
    ;
    int ctrl_chan = dma_claim_unused_channel(true);
    ;

    int data_chan2 = dma_claim_unused_channel(true);
    ;
    int ctrl_chan2 = dma_claim_unused_channel(true);
    ;

    // Setup the control channel for splash
    ...
    // Setup the control channel for boom
    dma_channel_config c3 = dma_channel_get_default_config(ctrl_chan2); // default configs
    channel_config_set_transfer_data_size(&3, DMA_SIZE_32);           // 32-bit txfers
    ...
    channel_config_set_chain_to(&c3, data_chan2);                       // chain to data channel
    ...
    // Setup the data channel for boom
    dma_channel_config c4 = dma_channel_get_default_config(data_chan2); // Default configs
    ...
    channel_config_set_chain_to(&c4, ctrl_chan2); // Chain to control channel
    ...
    dma_channel_configure(
        ...
        boom_audio,           // Number of transfers
        false                      // Don't start immediately.
    );

     // Setup the data channel for splash
