...
unsigned short splash_data[SPALSH_NUM_SAMPLES];
...
int main()
{
    ...
    int k;
    for (k = 0; k< SPALSH_NUM_SAMPLES; k++){
        splash_data[k] = DAC_config_chan_A | ((original_splash_data[k] >> 4) & 0x0FFF); //first convert to 12 bit then add config
    }
    ...
    dma_timer_set_fraction(0, 0, 15624); // 8K Hz
    ...
    dma_channel_configure(
        ...
        SPALSH_NUM_SAMPLES,           // Number of transfers
        false                      // Don't start immediately.
    );
